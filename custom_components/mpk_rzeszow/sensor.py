...
class MpkRzeszowSensor(Entity):
    ...
    def state(self):
        if self._departures_number is not None and self._departures_number > 0:
            ...
            return MpkRzeszowSensor.departure_to_str(dep)
        return None
    ...
    def update(self):
        now = datetime.now()
        data = MpkRzeszowSensor.get_data(self._stop_id)
        if data is None:
            return
        departures = data[0][0]
        parsed_departures = []
        ...
        self._departures_by_line = MpkRzeszowSensor.group_by_line(self._departures)
    ...
    def get_html_departures(self):
        ...
        for departure in self._departures:
            html = html + '<tr><td style="text-align: center; padding: 4px">{}</td></tr>\n'.format(
                MpkRzeszowSensor.departure_to_str(departure))
        html = html + '</table>'
        return html
    ...
    def get_stop_name(stop_id):
        data = MpkRzeszowSensor.get_data(stop_id)
        if data is None:
            return None
